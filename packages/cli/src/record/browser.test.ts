...
test("redacts sensitive browser input before crossing the host binding", async () => {
  ...
  try {
    ...
    await subscribeBrowserRecord(context, collector, { sensitiveFields: ["auth-secret-field"], targetAliases: { "credential-entry": "auth-secret-field" } });
    await page.locator("input").fill("do-not-store-this");
    await page.waitForTimeout(20);
    const events = collector.stop();
    assert.ok(events.length > 0);
    assert.ok(events.every((event) => event.target === "auth-secret-field"));
